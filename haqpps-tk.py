...
class myThread (threading.Thread):
    ...
    def run(self):
        ...
        #pps_amp_filt.run()
        #subprocess.call(["python","pps_amp_filt.py"])
...
class App:
    def __init__(self, master):
        ...
        Button(bottom_frame, text="Plot Rate", command=self.rate_plot).pack(side=RIGHT)
    ...
    def wave_scope(self):
        ...
        #myThread.exitFlag=0;
        #thread1 = myThread(1, "WvScope-"+str(len(self.threads)+1), 1)
        #thread1 = scopeThread(1, "ScopeTh-"+str(len(self.threads)+1), 1)
        #thread1.start()
        #self.threads.append(thread1)
        proc = subprocess.Popen(["python","pps_amp_filt.py"])
