...
def read_offsets( filename="offset.txt", x0=0.0 ):
    """
    ...
    def generate_offset_data( filename="offset.txt" ):
...
def plot_rate( tick_x, tick_y, offs_x, offs_y, fit_x, fit_fn, spy, g, h, watch_info ):
...
def main():
    ...
    offs_x, offs_y = read_offsets( "offset.txt", x0 )

    fit, fit_x, fit_fn, spy = calc_rate( offs_x, offs_y)

    upper_bound, lower_bound = calc_bounds( fit, fit_x, watch_info["limit"] )

    plot_rate( tick_x, tick_y, offs_x, offs_y, fit_x, fit_fn, spy, upper_bound, lower_bound, watch_info )
    quit()
